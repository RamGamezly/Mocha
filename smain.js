...
const config = require("./assets/jsons/config.jsons");
...
client.on("guildCreate", async (guild) => {
  try {
  console.log(`Someone added Chole Chan to their discord! ${guild.name} Member count: ${guild.memberCount} owned by: ${guild.owner.user.username}!`)
  const owner = guild.owner.user
  var guildMsg = [
      "Thanks for adding me to your server.",
      "```**1.** Mocha default prefix is `^`."
  ]
  owner.send(guildMsg)
} catch (err) {
  return;
...
client.on('guildDelete', (guild) => {
  console.log(`Someone removed Chole Chan from their discord! ${guild.name} Member count: ${guild.memberCount} owned by: ${guild.owner.user.username}!`)
